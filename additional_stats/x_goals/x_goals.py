...
from ..utils.utils import filter_columns, flip_teams
...
def get_input_and_output_from_game_datas(df: pd.DataFrame, proto: Game, filter_shots_only: bool = True) -> Tuple[np.ndarray, np.ndarray, List[Hit]]:
    ...
    players = [player for player in proto.players if "RLCS" not in player.name]
    ...
    for hit in hits:
        ...
        player = [player for player in players if player.name == player_name][0]
        ...
        for i in reversed(range(6)):
            try:
                frame = _df.loc[hit.frame_number - i, :]
                break
            except KeyError:
                continue
        ...
        if frame is None:
            x = 1
            print("CANNOT FIND FRAME FOR XGOAL CALC")
